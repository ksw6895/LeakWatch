...
- 대안: /docs/steps/step-11-billing-and-plans.md 에 “가격 리서치 작업”을 릴리즈 체크리스트로 포함한다(문서만으로도 실행 가능).
...
OpenAI 비용(월):
- NormalizeCost = Docs * (Tin/1e6 * Cin_in + Tout/1e6 * Cin_out)
- EmailCost = Emails * (Ein/1e6 * Cwr_in + Eout/1e6 * Cwr_out)
