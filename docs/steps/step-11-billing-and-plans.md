...
- [x] Billing API 구현: `GET /v1/billing/current`, `POST /v1/billing/subscribe`, `POST /v1/billing/webhooks`
- [x] Entitlement service 구현: 업로드/이메일 사용량 체크 + usage counter 기록
- [x] 업로드 제한 적용: `DocumentsService.createDocumentUpload`에서 FREE/STARTER/PRO limit enforcement
- [x] 이메일 발송 제한 적용: `ActionsController.approveByActionRequestId`에서 limit enforcement
- [x] FREE findings 제한 적용: `FindingsController.list`에서 상위 3개만 반환
- [x] Billing UI 추가: `apps/web/src/app/(embedded)/app/settings/billing/page.tsx`
- [x] API 테스트 추가: `apps/api/test/billing.spec.ts`
