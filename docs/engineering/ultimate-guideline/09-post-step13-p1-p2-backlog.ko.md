...
| P2-01   | 설치 앱 동기화 자동화(주기 스케줄 + 운영 알림)                         | `apps/worker/src/main.ts`, `apps/worker/src/jobs/installed-apps-sync.ts`, `apps/worker/src/queue.ts`, `packages/shared/src/queue.ts`, `apps/api/src/modules/auth/tenant-prisma.service.ts`                                                                                             | `ec30dfa`                                  |
| P2-04   | Assumption closure(1~4 검증 완료)                                      | `docs/engineering/ultimate-guideline/06-assumptions-and-validation.ko.md`, `apps/api/test/actions-flow.spec.ts`, `apps/api/test/billing.spec.ts`                                                                                                                                       | `30875d7`, `e0d5487`                       |
| P2-N    | report share/export 협업 흐름(PDF + revoke 포함)                       | `apps/api/src/modules/reports/reports.controller.ts`, `apps/api/src/modules/reports/reports.service.ts`, `apps/api/test/reports.spec.ts`, `apps/web/src/app/reports/shared/[token]/page.tsx`, `apps/web/src/app/(embedded)/app/reports/[id]/page.tsx`                                  | `734840b`, `841fcfc`, `9ebba95`, `469e37a` |
...
| P2-03   | inbound parsing 정확도 고도화 + 운영 피드백 루프                       | `apps/api/src/modules/mailgun/mailgun.service.ts`, `apps/api/src/modules/auth/tenant-prisma.service.ts`, `apps/api/src/modules/actions/actions.controller.ts`, `apps/api/test/actions-flow.spec.ts`, `apps/web/src/app/(embedded)/app/actions/page.tsx`                                | `e0d5487`                                  |
| P2-05   | 모바일/접근성/성능 자동 품질게이트(Playwright+axe+CI)                  | `apps/web/playwright.config.ts`, `apps/web/e2e/agency-quality-gates.spec.ts`, `.github/workflows/ci.yml`, `apps/web/package.json`                                                                                                                                                      | `e0d5487`                                  |
| P1-H    | 문서 상세(`/app/documents/[documentId]`) 실구현 + 다운로드 동선        | `apps/web/src/app/(embedded)/app/documents/[documentId]/page.tsx`, `apps/api/src/modules/documents/documents.controller.ts`, `apps/api/src/modules/documents/documents.service.ts`, `apps/web/src/components/uploads-panel.tsx`, `apps/web/src/app/(embedded)/app/leaks/[id]/page.tsx` | `efe88ba`, `627202a`                       |
...
- 잔여 보강:
  - 코드 범위 잔여 항목 없음. Shopify API 직접 installed-app 조회 스코프(S2)는 운영 검증 항목으로 분리 관리.
- 완료 조건:
...
- Shopify 파트너 심사/운영 단계에서 S2(설치 앱 목록 직접 조회 스코프) 최종 승인 여부는 별도 확인이 필요하다.
